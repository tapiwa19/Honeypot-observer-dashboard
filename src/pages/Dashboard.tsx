...
export function Dashboard({ onNavigate }: DashboardProps) {
  ...
  // ✅ FIX: fetchStats now trusts the SERVER for all counts directly
  // No more filtering sessions on the frontend — the server already did the work
  const fetchStats = useCallback(async () => {
    try {
      // ✅ Only call getDashboardStats — server now returns correct counts
      // (after the server fix: activeSessions = last 1h sessions, countriesDetected = real count)
      const statsResponse = await api.getDashboardStats();
      const data = statsResponse.data;
 
      console.log('📊 [Dashboard] Stats from server:', data);
 
      setStats(prev => ({
        totalAttacks: data.totalAttacks ?? prev.totalAttacks,
        changePercent: data.changePercent ?? prev.changePercent,
        // ✅ FIX 1: Trust server's activeSessions directly (no more frontend filter)
        activeSessions: data.activeSessions ?? prev.activeSessions,
        threatLevel: data.threatLevel ?? prev.threatLevel,
        // ✅ FIX 2: Trust server's countriesDetected directly
        countriesDetected: data.countriesDetected ?? prev.countriesDetected,
      }));
    } catch (error) {
  ...
  reconnection: true,
  reconnectionDelay: 500,        // ✅ FASTER (was 1000ms)
  reconnectionDelayMax: 2000,    // ✅ Cap max delay
  reconnectionAttempts: 20,      // ✅ More attempts
  timeout: 10000,                // ✅ 10s connection timeout
  forceNew: false,               // ✅ Reuse existing connection
  upgrade: true                  // ✅ Upgrade to WebSocket ASAP
});
    ...
    socketConnection.on('connect_error', (error) => {
      ...
      setConnectionErrorMsg(error?.message || String(error));
    });
    ...
    socketConnection.on('new_session', (newSession: any) => {
      ...
      setStats(prev => {
        ...
        return {
          ...
          // ✅ FIX 3: Don't manually increment activeSessions here
          // Instead we re-fetch from server below for accuracy
          threatLevel: newThreatLevel
        };
      });
 
      // Re-fetch from server to get accurate activeSessions + countriesDetected
    ...
    socketConnection.on('session_closed', (data: { sessionId: string }) => {
      ...
      // Re-fetch from server so the count is always accurate
      fetchStats();
    });
    ...
    // Fallback: if socket never connects, fetch manually
    const fallbackTimer = setTimeout(() => {
      if (!socketConnection.connected) {
        console.warn('[Dashboard] socket did not connect, fetching stats manually');
  ...
  // ✅ Periodic re-fetch every 30s as safety net (not main update mechanism)
  // This ensures counts are always accurate even if a WebSocket event was missed
  useEffect(() => {
  ...
  const statsCards: Array<{
  ...
  }> = [
    {
      ...
      change: stats.changePercent !== undefined
        ? (stats.changePercent >= 0 ? `+${stats.changePercent}%` : `${stats.changePercent}%`)
        : '…',
      trend: stats.changePercent !== undefined && stats.changePercent < 0 ? 'down' : 'up',
      icon: Shield,
            ...
            {liveIndicator ? (
              <div className="flex items-center gap-2 px-4 py-2 bg-green-50 border border-green-200 rounded-lg">
                ...
                <div className="flex flex-col">
                  ...
                  {connectionHealth.latency > 0 && (
                    <span className="text-green-600 text-xs">{connectionHealth.latency}ms latency</span>
                  )}
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2 px-4 py-2 bg-red-50 border border-red-200 rounded-lg">
                ...
                <div className="flex flex-col">
                  ...
                  {connectionErrorMsg && (
                    <span className="text-red-600 text-xs mt-1">Error: {connectionErrorMsg}</span>
                  )}
                </div>
              </div>
            )}
          ...
          <div className="mb-6 bg-red-50 border-2 border-red-500 rounded-xl p-6 shadow-lg animate-pulse">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className="text-red-900 font-bold text-lg mb-2">🚨 POSSIBLE DDoS ATTACK DETECTED!</h3>
                <p className="text-red-800 mb-3">
                  Detected {ddosAlert.suspiciousIPs.length} suspicious IP
                  {ddosAlert.suspiciousIPs.length !== 1 ? 's' : ''} with excessive request rates (
                  {ddosAlert.totalRequests.toLocaleString()} total requests in last minute).
                </p>
                  ...
                  <div className="space-y-2">
                    {ddosAlert.suspiciousIPs.slice(0, 5).map((item, i) => (
                      <div key={i} className="flex items-center justify-between text-sm">
                        <code className="text-red-700 font-mono bg-red-50 px-2 py-1 rounded">{item.ip}</code>
                        <span className="text-red-900 font-bold">{item.count.toLocaleString()} requests/min</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            return (
              <div
                ...
                className={`bg-white border rounded-xl p-5 hover:scale-[1.02] transition-all cursor-pointer shadow-sm ${
                  card.pulsing
                    ? 'border-[#00D9FF] ring-2 ring-[#00D9FF]/20 animate-pulse'
    ...
    <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full ${liveIndicator ? 'bg-green-500 animate-pulse' : 'bg-gray-400'}`} />
          ...
          alerts.map((alert, index) => (
            ...
            >
              <div className="flex items-start gap-3">
                <div
                  className={`px-2 py-1 rounded text-xs bg-gradient-to-r ${
