...
export default function Settings() {
  const [activeTab, setActiveTab] = useState<
    'general' | 'users' | 'cowrie' | 'alerts' | 'notifications' | 'integrations' | 'backup' | 'rules' | 'security'
  >('general');
  ...
  const addWebhook = () => {
    const newWebhook = { id: customWebhooks.length + 1, name: 'New Webhook', url: '', enabled: false };
  ...
  const handleCreateBackup = () => {
    ...
    setTimeout(() => showToastMessage('Backup created successfully!', 'success'), 2000);
  };
  ...
  const handleRestoreBackup = (id: number) => {
    if (confirm('Restore from this backup? Current data will be overwritten.')) {
      ...
      setTimeout(() => showToastMessage('Backup restored successfully!', 'success'), 2000);
    }
  };
  ...
  const handleImportConfig = () => {
    ...
    input.onchange = (e: Event) => {
      ...
      if (file) {
        ...
        setTimeout(() => showToastMessage('Configuration imported successfully!', 'success'), 1500);
      }
    };
    input.click();
  };
  ...
  const handleExportConfig = () => {
    const config = {
      ...
      integrations: { slackWebhook, slackEnabled, ntfyTopic, ntfyEnabled, smtpServer, smtpPort, customWebhooks }
    };
  ...
  const tabs = [
    ...
    { id: 'cowrie', label: 'Cowrie', icon: Server },
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" value={honeypotIP} onChange={(e) => setHoneypotIP(e.target.value)}
                  ...
                  <div>
                    ...
                    <input type="text" value={honeypotPort} onChange={(e) => setHoneypotPort(e.target.value)}
                      className="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#00D9FF]" />
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-gray-700 pt-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={elasticsearchURL} onChange={(e) => setElasticsearchURL(e.target.value)}
                      className="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#00D9FF]" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-400 mb-2">Kibana URL</label>
                    <input type="text" value={kibanaURL} onChange={(e) => setKibanaURL(e.target.value)}
                      className="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#00D9FF]" />
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'cowrie' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[
                  { label: 'SSH Port', value: sshPort, setter: setSshPort },
                  { label: 'Hostname', value: hostname, setter: setHostname },
                  { label: 'Max Concurrent Sessions', value: maxSessions, setter: setMaxSessions, type: 'number' },
                  { label: 'Session Timeout (seconds)', value: sessionTimeout, setter: setSessionTimeout, type: 'number' },
                ...
                <div>
                  ...
                  <select value={filesystemTemplate} onChange={(e) => setFilesystemTemplate(e.target.value)}
                    className="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#00D9FF]">
              ...
              <div>
                ...
                <textarea value={welcomeBanner} onChange={(e) => setWelcomeBanner(e.target.value)} rows={4}
              ...
              <div className="flex justify-end">
                <button onClick={() => handleSaveSettings('Cowrie')}
                  className="px-6 py-2 bg-[#00D9FF] text-white font-bold rounded-lg hover:bg-[#00D9FF]/80 transition">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={alert.key} className={`flex items-center justify-between p-4 bg-${alert.color}-500/10 rounded-lg border border-${alert.color}-500/30`}>
                      ...
                      <input type="checkbox" checked={alert.state} onChange={(e) => alert.setState(e.target.checked)}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-900/60 rounded-lg p-6 border border-gray-700">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 bg-purple-500 rounded-lg flex items-center justify-center text-white font-bold text-lg">S</div>
                    <div>
                ...
                <div className="space-y-3">
                  <input type="text" value={slackWebhook} onChange={(e) => setSlackWebhook(e.target.value)}
                    ...
                    className="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#00D9FF]" />
                  <button onClick={() => handleTestIntegration('Slack')}
              ...
              <div className="bg-gray-900/60 rounded-lg p-6 border border-gray-700">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 bg-[#00D9FF]/20 border border-[#00D9FF]/40 rounded-lg flex items-center justify-center">
                      <Smartphone className="w-6 h-6 text-[#00D9FF]" />
                    </div>
                    <div>
                      <h3 className="font-bold text-white">Ntfy Push Notifications</h3>
                ...
                <div className="space-y-3">
                  <div>
                    <label className="block text-sm font-medium text-gray-400 mb-1">Ntfy Topic Name</label>
                    <input type="text" value={ntfyTopic} onChange={(e) => setNtfyTopic(e.target.value)}
                      placeholder="e.g. honeypot-observer-alerts"
                      className="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white font-mono focus:outline-none focus:border-[#00D9FF]" />
                    <p className="text-xs text-gray-500 mt-1">
                      Subscribe to{' '}
                      <span className="text-[#00D9FF] font-mono">ntfy.sh/{ntfyTopic || 'your-topic'}</span>
                      {' '}in the Ntfy app to receive alerts
                    </p>
                  </div>
                  <button onClick={() => handleTestIntegration('Ntfy')}
                    className="px-4 py-2 bg-[#00D9FF] text-white rounded-lg hover:bg-[#00D9FF]/80 transition text-sm">
              ...
              <div className="bg-gray-900/60 rounded-lg p-6 border border-gray-700">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-12 h-12 bg-red-500 rounded-lg flex items-center justify-center text-white font-bold text-lg">@</div>
                  <div>
                ...
                <button onClick={() => handleTestIntegration('SMTP')}
                    ...
                    <div key={webhook.id} className="bg-gray-900/60 rounded-lg p-4 border border-gray-700">
                      <div className="flex items-center gap-3 mb-3">
                        <input type="text" value={webhook.name}
                          onChange={(e) => setCustomWebhooks(customWebhooks.map(w => w.id === webhook.id ? { ...w, name: e.target.value } : w))}
                        ...
                        <input type="checkbox" checked={webhook.enabled}
                          onChange={(e) => setCustomWebhooks(customWebhooks.map(w => w.id === webhook.id ? { ...w, enabled: e.target.checked } : w))}
                          className="w-5 h-5 accent-[#00D9FF] cursor-pointer" />
                        <button onClick={() => removeWebhook(webhook.id)}
                          className="p-2 bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30 transition">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                      <input type="text" value={webhook.url}
                        onChange={(e) => setCustomWebhooks(customWebhooks.map(w => w.id === webhook.id ? { ...w, url: e.target.value } : w))}
                        className="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#00D9FF]"
              ...
              <div className="bg-gray-800/90 rounded-lg border border-gray-700 p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select value={backupFrequency} onChange={(e) => setBackupFrequency(e.target.value)} disabled={!autoBackup}
                    ...
                    <thead className="bg-gray-900/60 border-b border-gray-700">
                      <tr>
                        {['Date & Time', 'Size', 'Status', 'Actions'].map(h => (
                          <th key={h} className="px-6 py-3 text-left text-xs font-bold text-gray-400 uppercase">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-700">
                      {backupHistory.map((backup) => (
                        <tr key={backup.id} className="hover:bg-gray-700/30">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-400">{backup.size}</td>
                          <td className="px-6 py-4 whitespace-nowrap">
          ...
          {activeTab === 'security' && (
            <div className="space-y-8">
              <div>
                ...
                <p className="text-gray-400 text-sm">Manage your password and dangerous system operations</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {/* Change Password */}
                <div className="bg-gray-900/60 rounded-lg border border-gray-700 p-6">
                ...
                {/* Danger Zone */}
                <div className="bg-gray-900/60 rounded-lg border border-red-500/30 p-6">
                  <h3 className="font-bold text-white mb-4 flex items-center gap-2">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-red-400 mb-3">
                          Permanently deletes all captured attack data, sessions, and logs. This cannot be undone.
                        </p>
                        <button
                          onClick={() => {
